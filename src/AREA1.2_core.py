...
def permute_area_norm(ar_ticks, permutations=1000, seed=42, verbose=False):
    '''Generates permutations of the ranks and calculates AUC for each
        permutation.'''
    if args.verbose:
        print("ar_ticks", ar_ticks)
    ...
    for i in range(permutations):
        ...
        es_permute_score = area_score_norm(random_ar_score, verbose=verbose)  # Use area_score_norm
...
def process_balabel(balabel, todolist, outdir, commoncolumn, rankorderdf, badf,keepsamples,use_gpu, verbose=False):
    ...
    baNESpvals = run_a_ba(balabel, valuecols, outdir, commoncolumn, rankorderdf, badf,keepsamples,use_gpu, verbose=verbose)
    return baNESpvals

def org_to_pval(outdir, orgfile, commoncolumn, rankorderdf, badf, keepsamples, use_gpu,n_processes=4, verbose=False):
    print ("collecting pvalues")
    if use_gpu:
        import cupy as array_lib
    else:
        import numpy as array_lib
    ...
    with Pool(processes=n_processes) as pool:
        ...
        results = pool.starmap(process_balabel, [(balabel, todolist, outdir, commoncolumn, rankorderdf, badf,keepsamples,use_gpu,verbose) for balabel in todolist["balabel"].unique()])
...
def run_a_ba(ba,valuecols, outdir, commoncolumn, rankorderdf, badf, keepsamples,use_gpu, verbose=False):
    if verbose:
        print(f"Running BA {ba} on {len(valuecols)} value columns")
    if verbose:
        print(f"run_a_ba: {ba}")
        print(f"  badf shape: {badf.shape}")
        print(f"  rankorderdf shape: {rankorderdf.shape}")
        print(f"  keepsamples len: {len(keepsamples)}")
        print(ba)
        print(commoncolumn)
        print(valuecols)
    ...
    if verbose:
        print("what to merge")
        print(subbadf.head)
        print(subrankorderdf.head)
        print("mergedf", mergedf.shape)
    ...
    simES_norm = permute_area_norm(thiscomorbidity_binary, permutations=1000, verbose=verbose)
    lines = []
...
#                            help='Minimum individual expression threshold (default: 10)')
    ...
    org_to_pval(outdir, orgfile, commoncolumn, rankorderdf, badf, keepsamples=keepsamples,n_processes=n_processes,use_gpu=args.gpu, verbose=args.verbose)
